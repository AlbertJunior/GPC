...
public:
	...
	void WritePixel(int linie, int coloana, int raza) {
		...
		if (linie < 1 || linie > this->m || coloana < 1 || coloana > this->n) {
			return;
		}
		...
		double x_ecran = -1 + epsilon + (linie - 1) * distantaColoane;
		double y_ecran = -1 + epsilon + (coloana - 1) * distantaLinii;
		glBegin(GL_POLYGON);
	...
	void CalculeazaDistantaIntreLinii() {
		...
		this->distantaLinii = (double)((2 - 2 * epsilon) / (n - 1));
		this->distantaColoane = (double)((2 - 2 * epsilon) / (m - 1));
	}
...
public:
	...
	void WriteLine(GrilaCarteziana grilaCarteziana, int l1, int c1, int l2, int c2) {
		...
		double x_ecran1 = -1 + grilaCarteziana.epsilon + (l1 - 1) * grilaCarteziana.distantaColoane;
		double y_ecran1 = -1 + grilaCarteziana.epsilon + (c1 - 1) * grilaCarteziana.distantaLinii;
		double x_ecran2 = -1 + grilaCarteziana.epsilon + (l2 - 1) * grilaCarteziana.distantaColoane;
		double y_ecran2 = -1 + grilaCarteziana.epsilon + (c2 - 1) * grilaCarteziana.distantaLinii;
		glColor3f(1, 0, 0);
		glBegin(GL_LINES);
	...
	void WriteLinePixels(GrilaCarteziana grilaCarteziana, int l1, int c1, int l2, int c2, int pattern_flag) {
		...
		else if (l2 >= l1 && c2 >= c1 && (c2 - c1) < (l2 - l1)) {
			int dx = c2 - c1;
			int dy = l2 - l1;
			int d = 2 * dx - dy;
			int dN = 2 * dx;
...
void Display(void)
{
	...
	GrilaCarteziana grilaCarteziana(15, 30);
	Desenare2D desenare2D;

	desenare2D.WriteLine(grilaCarteziana, 1, 1, 8, 16);
	desenare2D.WriteLinePixels(grilaCarteziana, 1, 1, 8, 16, 0);
	desenare2D.WriteLine(grilaCarteziana, 16, 1, 11, 16);
	desenare2D.WriteLinePixels(grilaCarteziana, 16, 1, 11, 16, 1);

	desenare2D.WriteLine(grilaCarteziana, 6, 25, 1, 22);
	desenare2D.WriteLinePixels(grilaCarteziana, 6, 25, 1, 22, 2);

	desenare2D.WriteLine(grilaCarteziana, 15, 21, 9, 25);
	desenare2D.WriteLinePixels(grilaCarteziana, 15, 21, 9, 25, 4);
	glFlush();
}
